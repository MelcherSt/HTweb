...
class Controller_Sessions extends \Controller_Gate {
	...
	public function post_enroll($date=null) {
		...
		if(isset($date)&& \Utils::valid_date($date)) {
			...
			if(!$session) {
				\Utils::handle_irrecoverable_error('Session does not exist.');
			}
	...
	/**
	 * Deletes the currentuser's enrollment for given session
	 * @param type $date
	 * @throws \HttpNotFoundException
	 */
	public function delete_enroll($date=null) {
		if(isset($date) && \Utils::valid_date($date)) {
			...
			if(!$session) {
				\Utils::handle_irrecoverable_error('Unable to leave non-existant session.');
			} 
